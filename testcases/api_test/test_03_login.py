...
class TestUserLogin():
    ...
    # @allure.issue("http://localhost:8009/api/auth/login", name="点击，跳转到对应BUG的链接地址")
    # @allure.testcase("http://localhost:8009/api/auth/login", name="点击，跳转到对应用例的链接地址")
    # @allure.title("测试数据：【 {username}，{passwordz}，{except_result}，{except_code}，{except_msg}】")
    # @pytest.mark.single
